...
import { MapMarkerArg } from './map/Marker';
import { MapCourseArg as CourseArg } from './map/Course';
import { RoutePlannerArg } from './map/RoutePlanner';
...
const DynamicMapMarker = dynamic<MapMarkerArg>(() => import('./map/Marker'), {
	ssr: false,
});
const DynamicCourse = dynamic<CourseArg>(() => import('./map/Course'), {
	ssr: false,
});
const DynamicRoutePlanner = dynamic<RoutePlannerArg>(() => import('./map/RoutePlanner'), {
	ssr: false,
});
const DynamicEle = dynamic<EleArg>(() => import('./map/Ele'), { ssr: false });
...
export type RoutePlannerPanelProps = {
	/** Called whenever the active course or its name changes. */
	onCourseChange?: (course: CourseData | null, courseName: string) => void;
	/** Called whenever the edit (route planning) mode changes. */
	onEditModeChange?: (editMode: boolean) => void;
};

export default function RoutePlannerPanel({ onCourseChange, onEditModeChange }: RoutePlannerPanelProps) {
